...
public class Tetromino {
   ...
   public boolean rotate(String direction, GameGrid gameGrid) {
	   if(!canBeMoved(direction, gameGrid))
		   return false;
	   Tile[][] temp = new Tile[n][n];
	   
	   for (int i = 0; i < n; i++)
		   for(int k = 0; k < n; k++)
			   temp[i][k] = tileMatrix[i][k];
	   ...
	   case('O'):
		   	rotateI(temp);
	   		break;		//for now it uses rotateI
	   }
	   
	   //rotate points? (clear old point create new tile with rotated angle ?) (try transpose for rotating)
	   return true;
   }

   private void rotateI(Tile[][] temp) {
	   for (int i = 0; i < n; i++)
		   for(int k = 0; k < n; k++)			//Fine I guess?
			   tileMatrix[k][i] = temp[i][k];
	
   }
   
   private void rotateZ(Tile[][] temp) {
	   for (int i = 0; i < n; i++)
		   for(int k = 0; k < n; k++)
			   tileMatrix[k][i] = temp[i][k];		//Fine I guess?
	
   }
   
   private void rotateT(Tile[][] temp) {
	   if(count == 0) {
	   for (int i = 0; i < n; i++)
		   for(int k = 0; k < n; k++)
			   tileMatrix[k][i] = temp[i][k];		//WIP
	   	count = 1;
	   }
	   else if(count == 1) {
		   for (int i = 0; i < n; i++)
			   for(int k = 0; k < n; k++)
				   tileMatrix[k][i] = temp[n - 1 - i][n - 1 - k];		
		   	count = 0;
	   }
		   
   }

   private void rotateL(Tile [][] temp) {
	   
	   System.out.println("count:" + count);							//work on this idea later () it was n == 4 here before
	   if(count == 0) {
		   for (int i = 0; i < n; i++)
			   for(int k = 0; k < n; k++)
				   tileMatrix[n-1-k][i] = temp[i][k];		
		   count = 1;
		   }
		   else if(count == 1) {
			   ...
			   for (int i = 0; i < n; i++)
				   for(int k = 0; k < n; k++)
					   tileMatrix[n-1-i][k] = temp[n-1-k][i];		   
			   	count = 0;
		   }
   }
